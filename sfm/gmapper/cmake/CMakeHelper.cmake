...
set(GCOLMAP_TARGETS_ROOT_FOLDER "gcolmap_targets")
set(GCOLMAP_SRC_ROOT_FOLDER "gcolmap_sources")
...
macro(GCOLMAP_ADD_SOURCE_DIR SRC_DIR SRC_VAR)
    ...
    unset(ARG)
    unset(GROUP_NAME)
endmacro(GCOLMAP_ADD_SOURCE_DIR)

# Macro to add source files to GCOLMAP library.
macro(GCOLMAP_ADD_SOURCES)
    set(SOURCE_FILES "")
    ...
    endforeach()
    set(GCOLMAP_SOURCES ${GCOLMAP_SOURCES} ${SOURCE_FILES} PARENT_SCOPE)
endmacro(GCOLMAP_ADD_SOURCES)

# Macro to add CUDA source files to GCOLMAP library.
macro(GCOLMAP_ADD_CUDA_SOURCES)
    set(SOURCE_FILES "")
    foreach(SOURCE_FILE ${ARGN})
        if(SOURCE_FILE MATCHES "^/.*")
            # Absolute path.
    ...
    set(GCOLMAP_CUDA_SOURCES
        ${GCOLMAP_CUDA_SOURCES}
        ${SOURCE_FILES}
        PARENT_SCOPE)
endmacro(GCOLMAP_ADD_CUDA_SOURCES)
...
macro(GCOLMAP_ADD_LIBRARY TARGET_NAME)
    # ${ARGN} will store the list of source files passed to this function.
    add_library(${TARGET_NAME} ${ARGN})
    set_target_properties(${TARGET_NAME} PROPERTIES FOLDER
        ${GCOLMAP_TARGETS_ROOT_FOLDER}/${FOLDER_NAME})
    install(TARGETS ${TARGET_NAME} DESTINATION lib/gcolmap/)
endmacro(GCOLMAP_ADD_LIBRARY)
macro(GCOLMAP_ADD_STATIC_LIBRARY TARGET_NAME)
    # ${ARGN} will store the list of source files passed to this function.
    add_library(${TARGET_NAME} STATIC ${ARGN})
    set_target_properties(${TARGET_NAME} PROPERTIES FOLDER
        ${GCOLMAP_TARGETS_ROOT_FOLDER}/${FOLDER_NAME})
    install(TARGETS ${TARGET_NAME} DESTINATION lib/gcolmap)
endmacro(GCOLMAP_ADD_STATIC_LIBRARY)
...
macro(GCOLMAP_ADD_CUDA_LIBRARY TARGET_NAME)
    # ${ARGN} will store the list of source files passed to this function.
    cuda_add_library(${TARGET_NAME} ${ARGN})
    set_target_properties(${TARGET_NAME} PROPERTIES FOLDER
        ${GCOLMAP_TARGETS_ROOT_FOLDER}/${FOLDER_NAME})
    install(TARGETS ${TARGET_NAME} DESTINATION lib/gcolmap/)
endmacro(GCOLMAP_ADD_CUDA_LIBRARY)
macro(GCOLMAP_ADD_STATIC_CUDA_LIBRARY TARGET_NAME)
    # ${ARGN} will store the list of source files passed to this function.
    cuda_add_library(${TARGET_NAME} STATIC ${ARGN})
    set_target_properties(${TARGET_NAME} PROPERTIES FOLDER
        ${GCOLMAP_TARGETS_ROOT_FOLDER}/${FOLDER_NAME})
    install(TARGETS ${TARGET_NAME} DESTINATION lib/gcolmap/)
endmacro(GCOLMAP_ADD_STATIC_CUDA_LIBRARY)

# Replacement for the normal add_executable() command. The syntax remains the
# same in that the first argument is the target name, and the following
# arguments are the source files to use when building the target.
macro(GCOLMAP_ADD_EXECUTABLE TARGET_NAME)
    # ${ARGN} will store the list of source files passed to this function.
    add_executable(${TARGET_NAME} ${ARGN})
    set_target_properties(${TARGET_NAME} PROPERTIES FOLDER
        ${GCOLMAP_TARGETS_ROOT_FOLDER}/${FOLDER_NAME})
    target_link_libraries(${TARGET_NAME} gcolmap)
    if(VCPKG_BUILD)
        install(TARGETS ${TARGET_NAME} DESTINATION tools/)
    else()
        install(TARGETS ${TARGET_NAME} DESTINATION bin/)
    endif()
endmacro(GCOLMAP_ADD_EXECUTABLE)

# Wrapper for test executables.
macro(GCOLMAP_ADD_TEST TARGET_NAME)
    if(TESTS_ENABLED)
        # ${ARGN} will store the list of source files passed to this function.
        add_executable(${TARGET_NAME} ${ARGN})
        set_target_properties(${TARGET_NAME} PROPERTIES FOLDER
...
endmacro(GCOLMAP_ADD_TEST)

# Wrapper for CUDA test executables.
macro(GCOLMAP_ADD_CUDA_TEST TARGET_NAME)
    if(TESTS_ENABLED)
        # ${ARGN} will store the list of source files passed to this function.
...
endmacro(GCOLMAP_ADD_CUDA_TEST)
