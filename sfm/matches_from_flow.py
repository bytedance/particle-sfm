...
def traj_to_matches(img_dir, traj_dir, match_list_file, remove_dynamic=True):
    ...
    for key in tqdm(trajectories):
        ...
        locations, labels, frame_ids = np.array(traj["locations"]), np.array(traj["labels"]), np.array(traj["frame_ids"])
            ...
            if len(img_ids) <= sample_k:
                ...
                for k in range(sample_k):
                    ...
                    if tgt_img_traj_ind == j:
                        continue
                    image_datas[img_ids[j]].insert_match(img_ids[tgt_img_traj_ind], kp_inds[j], kp_inds[tgt_img_traj_ind])
